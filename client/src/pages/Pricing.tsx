...
const Pricing = () => {
    ...
    const getCtaText = (planName: string) => {
        if (planName.toLowerCase() === 'basic') return 'Start Building';
        if (planName.toLowerCase() === 'pro') return 'Upgrade to Pro';
    ...
    <>
      <div className='w-full max-w-5xl mx-auto z-20 max-md:px-4 min-h-[80vh]'>
        <div className='text-center mt-16'>
            <h2 className='text-gray-100 text-3xl font-medium'>Pricing That Grows With You</h2>
            <p className='text-gray-400 text-sm max-w-md mx-auto mt-2'>
                Start free. Upgrade when you need more AI power to build and scale websites.
            </p>
        </div>
        ...
        <div className='grid grid-cols-1 md:grid-cols-3 flex-wrap gap-4'>
                ...
                <p className='mx-auto text-center text-sm max-w-md mt-10 text-white/60 font-light'>Each <span className='text-white'> website creation or revision</span> uses <span className='text-white'>5 credits </span>. Need more? You can upgrade or purchase additional credits anytime.</p>
      </div>
      <Footer />
    </>
  )
